 ...
 * sub package and prefixed with 'Paros'
 *
 * @author psiinon
 */
public interface Database {
     ...
     * database will be compacted, compacting the database ensures a minimal space
     * disk usage but it will also take longer to close. Any necessary cleanups are
     * performed prior to closing the database (the cleanup removes the temporary
     * {@code HistoryReference}s).
     *
     * <p>
     * <b>Note:</b> Calling this method has the same effects as calling the method
     * {@link #close(boolean, boolean)} with the parameter {@code cleanup} as
     * {@code true}.
     *
     * @param compact {@code true} if the database should be compacted,
     ...
     * Closes the database. If the parameter {@code compact} is {@code true}, the
     * database will be compacted, compacting the database ensures a minimal space
     * disk usage but it will also take longer to close. If the parameter
     * {@code cleanup} is {@code true} any necessary cleanups are performed prior to
     * closing the database (the cleanup removes the temporary {@code
     * HistoryReference}s.)
     *
     * @param compact {@code true} if the database should be compacted,
     *                {@code false} otherwise.
     * @param cleanup {@code true} if any necessary cleanups should be performed,
     *                {@code false} otherwise.
     * @see org.parosproxy.paros.model.HistoryReference
     */
    ...
    /**
     * Discards the history associated with the given session ID, called when a
     * session is not to be saved (and a new one is about to be created).
     *
     * <p>
     * Implementations might opt to do nothing, for example, if the database is file
     * based (HSQLDB) and those files are deleted if the session is not saved.
     *
     * @param sessionId the ID of the session
     * @throws DatabaseException If an error occurred while discarding the history
     *                           of the session.
     * @since 2.5.0
     */
