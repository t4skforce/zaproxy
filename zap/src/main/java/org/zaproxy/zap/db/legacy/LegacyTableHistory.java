...
import org.zaproxy.zap.db.schema.History.HistoryBuilder;
import org.zaproxy.zap.network.HttpRequestBody;
import org.zaproxy.zap.network.HttpResponseBody;

@Service
public class LegacyTableHistory implements TableHistory {
                ...
                .filter(history -> {
                    if (isRequest) {
                        return pattern.matcher(history.getRequestHeader()).find()
                                || pattern.matcher(history.getRequestBody()).find();
                    }
                    return pattern.matcher(history.getResponseHeader()).find()
                            || pattern.matcher(history.getResponseBody()).find();
                })
    ...
    public RecordHistory getHistoryCache(HistoryReference ref, HttpMessage reqMsg)
        ...
        List<History> history = historyRepository.findAllHistoryCache((long) ref.getHistoryId(),
    ...
    public int lastIndex() {
        return historyRepository.findTopByOrderByIdDesc().map(history -> history.getId()).orElse(0L).intValue();
    }
