...
/**
 * Class that contains/provides all built-in (core) components (i.e. extensions
 * and active/passive scanners).
 *
 * <p>
 * This class means we don't have to search the core jar for components which
 * was significantly impacting the start time (along with search in add-ons).
 *
 * <p>
 * <strong>Note:</strong> If you add a new 'core' component then you will need
 * to add it to this class (in alphabetic order please). Note that ideally we
 * would prefer new functionality to be defined in add-ons as this means we can
 * update them without 'full' releases. The lists could have been maintained in
 * a manifest file as per the add-ons, but having it as code means that it's
...
public final class CoreFunctionality {
    ...
    private static synchronized void createExtensions() {
        if (builtInExtensions == null) {
            ...
            extensions.add(
                    new org.zaproxy.zap.extension.httppanel.view.largerequest.ExtensionHttpPanelLargeRequestView());
            extensions.add(
                    new org.zaproxy.zap.extension.httppanel.view.largeresponse.ExtensionHttpPanelLargeResponseView());
            extensions.add(
                    new org.zaproxy.zap.extension.httppanel.view.paramtable.ExtensionHttpPanelRequestFormTableView());
            extensions.add(
                    new org.zaproxy.zap.extension.httppanel.view.paramtable.ExtensionHttpPanelRequestQueryCookieTableView());
            extensions.add(new org.zaproxy.zap.extension.httppanel.view.posttable.ExtensionRequestPostTableView());
            extensions.add(
                    new org.zaproxy.zap.extension.httppanel.view.syntaxhighlight.ExtensionHttpPanelSyntaxHighlightTextView());
